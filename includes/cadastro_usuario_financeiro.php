...
<?php if ( $usuariosistema->pflcod ): ?>
	...
	<?php
		$sql = sprintf(
			"select distinct u.unicod, u.unidsc from public.unidade u inner join financeiro.unidproposto up using ( unicod ) where up.usucpf = '%s'",
			$usucpf
		);
				...
				<?php foreach( $perfisUsuario as $perfil ): ?>
					...
					<tr bgcolor="<?=$marcado?>">
						<td style="color: #003c7b">
							<? if ($mostraMais): ?>
								<a href="Javascript:abreconteudo('../geral/cadastro_usuario_financeiro_responsabilidades.php?usucpf=<?=$usucpf?>&pflcod=<?=$perfil["pflcod"]?>','<?=$perfil["pflcod"]?>')">
		...
		switch( tprsigla ){			
			case 'U':
				abreresp = window.open(
					"../geral/cadastro_usuario_financeiro_unidade.php?pflcod="+pflcod+"&usucpf=<?=$usucpf?>",
