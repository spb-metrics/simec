...
class DisplayValue {
	...
	function SetFormatCallbackParam($aFunc, &$param) {
        $this->iFormCallback = $aFunc;
        $this->iFormCallbackParam = $param;
    }
    
	function SetTypeGraph($name) {
        $this->typeGraph = $name;
    }
	...
	if( $this->show ) 
	{
	    ...
	    if( $this->iFormCallback != '' ) {
			...
			if(is_array($this->iFormCallbackParam) && count($this->iFormCallbackParam)){
				$sval = call_user_func($f,$aVal, $this->iFormCallbackParam[$this->typeGraph][$aVal]);				
			} else {
