...
<tr>
	...
	<td>
		<?php
			...
			//dbg( $sql_perfil, 0 , '03700155689' );
			//dbg( $db->carregar( $sql_perfil ) );
			$sql = sprintf(
				...
				<?php foreach( $perfisUsuario as $perfil ): ?>
					...
					<tr bgcolor="<?=$marcado?>">
						<td style="color: #003c7b">
							<? if ($mostraMais): ?>
								<a href="Javascript:abreconteudo('../geral/cadastro_usuario_elaboracao_responsabilidades.php?usucpf=<?=$usucpf?>&pflcod=<?=$perfil["pflcod"]?>','<?=$perfil["pflcod"]?>')">
		...
		switch( tprsigla ){
			case 'A':
				abreresp = window.open(
					"../geral/cadastro_usuario_elaboracao_acao.php?acao=A&pflcod="+pflcod+"&usucpf=<?=$usucpf?>",
			...
			case 'P':
				abreresp = window.open(
					"../geral/cadastro_usuario_elaboracao_programa.php?pflcod="+pflcod+"&usucpf=<?=$usucpf?>",
					"popresp_<?= $sistema->sisid ?>",
					"menubar=no,location=no,resizable=no,scrollbars=no,status=yes,width=500,height=480");		
				break;
			case 'U':
				abreresp = window.open(
					"../geral/cadastro_usuario_elaboracao_unidade.php?pflcod="+pflcod+"&usucpf=<?=$usucpf?>",
					"popresp_<?= $sistema->sisid ?>",
					"menubar=no,location=no,resizable=no,scrollbars=no,status=yes,width=500,height=480");		
				break;
		}
