				...
				<?php foreach( $perfisUsuario as $perfil ): ?>
					...
					<tr bgcolor="<?=$marcado?>">
						<td style="color: #003c7b">
							<? if ($mostraMais): ?>
								<a href="Javascript:abreconteudo('../geral/cadastro_usuario_reuni_responsabilidades.php?usucpf=<?=$usucpf?>&pflcod=<?=$perfil["pflcod"]?>','<?=$perfil["pflcod"]?>')">
		...
		switch( tprsigla ){
			case 'U':
				abreresp = window.open(
					"../geral/cadastro_usuario_reuni_unidade.php?pflcod="+pflcod+"&usucpf=<?=$usucpf?>",
