...
<?php
	$sql = "SELECT * FROM  cte.tiporesponsabilidade WHERE tprsnvisivelperfil = 't' ORDER BY tprdsc";
		...
		<td>
			<table border="0" cellpadding="2" cellspacing="0" width="500" class="listagem" bgcolor="#fefefe">
				...
				<?php foreach( $perfisUsuario as $perfil ): ?>
					<?php
						$marcado = $i++ % 2 ? '#F7F7F7' : '';
						$sqlResponsabilidadesPerfil = "SELECT p.*, tr.tprdsc, tr.tprsigla FROM (SELECT * FROM cte.tprperfil WHERE pflcod = '%s') p RIGHT JOIN cte.tiporesponsabilidade tr ON p.tprcod = tr.tprcod WHERE tprsnvisivelperfil = TRUE ORDER BY tr.tprdsc";
