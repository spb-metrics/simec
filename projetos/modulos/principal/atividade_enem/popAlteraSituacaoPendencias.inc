...
function validar(atiid, opcao, tipo, id, entid, opcao_evidencia, evidencia)
{
	var html = '';
	var observacao = '';
	var upload = '';
	...
	{
		html = 	'<input type="hidden" id="atiid" value="'+atiid+'" />' +
	...
	else
	{
		...
		html = '<span class="fechardiv" style="cursor:pointer;float:right;font-size:8px;" onmouseover="sublinhar(this,true);" onmouseout="sublinhar(this,false);" onclick="self.close();">[x]Fechar</span>' +
			   ...
			   '</tr>' +
			   '</table>' +
			   '<input type="hidden" id="atiid" value="'+atiid+'" />' +
			   '<input type="hidden" id="tipo" value="'+tipo+'" />' +
			   '<input type="hidden" id="opcao" value="'+opcao+'" />' +
			   '<input type="hidden" id="id" value="'+id+'" />' +
			   '<input type="hidden" id="entid" value="'+entid+'" />';
...
function salvar(opcao_evidencia,evidencia)
{
	...
	var atiid 	= document.getElementById('atiid').value;
	...
	var item = window.opener.document.getElementById('item_'+atiid+'_'+id+'_'+tipo);
	...
	var req = new Ajax.Request('enem.php?modulo=principal/atividade_enem/minhasPendencias&acao=A', 
	{
        method:     'post',
        ...
        onComplete: function (res)
        {
			if( res.responseText == 'ok' )
			{
				if( vldsituacao == 't' )
				{
					item.innerHTML = '<img src="/imagens/check_checklist.png" border="0" style="cursor:pointer;width:30px;height:30px;" onclick="alterarSituacao('+atiid+', \''+tipo+'\', \'true\', '+id+', '+entid+', \''+opcao_evidencia+'\', \''+evidencia+'\');" />';
				...
				{
					item.innerHTML = '<img src="/imagens/erro_checklist.png" border="0" style="cursor:pointer;width:30px;height:30px;" onclick="alterarSituacao('+atiid+', \''+tipo+'\', \'false\', '+id+', '+entid+', \''+opcao_evidencia+'\', \''+evidencia+'\');" />';
...
<?php if( $_GET['situacao'] == "nulo" ): ?>
	...
	<br /><br /><br />
	<b>Deseja validar o item?</b> 
	<br /><br />
	<input type="button" value="Sim" onclick="validar(<?=$_GET['atiid']?>, true, '<?=$_GET['tipo']?>', <?=$_GET['iclid']?>, <?=$_GET['entid']?>, '<?=$_GET['opcao_evidencia']?>', '<?=$_GET['evidencia']?>');" />
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<input type="button" value="Não" onclick="validar(<?=$_GET['atiid']?>, false, '<?=$_GET['tipo']?>', <?=$_GET['iclid']?>, <?=$_GET['entid']?>, '<?=$_GET['opcao_evidencia']?>', '<?=$_GET['evidencia']?>');" />
<?php endif; ?>

<?php if( $_GET['situacao'] == "true" ): ?>
	<span class="fechardiv" style="cursor:pointer;float:right;font-size:8px;" onmouseover="sublinhar(this,true);" onmouseout="sublinhar(this,false);" onclick="self.close();">[x]Fechar</span>
	<br /><br /><br />
	<b>Deseja invalidar o item?</b>
	<br /><br />
	<input type="button" value="Sim" onclick="validar(<?=$_GET['atiid']?>, false, '<?=$_GET['tipo']?>', <?=$_GET['iclid']?>, <?=$_GET['entid']?>, '<?=$_GET['opcao_evidencia']?>', '<?=$_GET['evidencia']?>');" />
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
