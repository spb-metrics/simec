	...
	foreach ($responsabilidadesPerfil as $rp) {
		...
		switch ($rp["tprsigla"]) {
			case "U": // Unidades 
				...
				$sqlRespUsuario = "SELECT DISTINCT u.unicod AS codigo, u.unidsc AS descricao, ur.rpustatus AS status
			...
			case "E": // Estados
				...
				$sqlRespUsuario = "SELECT DISTINCT e.estuf AS codigo, e.estdescricao AS descricao, ur.rpustatus AS status
					FROM obras.usuarioresponsabilidade ur 
			...
			case "O": // Órgão
				...
				$sqlRespUsuario = "
					SELECT DISTINCT
						o.orgcodigo AS codigo, o.orgdesc AS descricao
					FROM 
						obras.orgao AS o 
					INNER JOIN 
