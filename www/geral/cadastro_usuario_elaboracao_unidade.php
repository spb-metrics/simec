...
if(is_array($_POST['usuunidresp']) && @count($_POST['usuunidresp'])>0) {
	...
	$sqlSelResp = "SELECT distinct ur.rpuid, ur.usucpf, ur.rpustatus, u.unidsc, u.unicod, u.unicod FROM elabrev.usuarioresponsabilidade ur 
	...
	$sqlInsRpu = "INSERT INTO elabrev.usuarioresponsabilidade (unicod, usucpf, rpustatus, rpudata_inc, pflcod,prsano) VALUES ('%s', '%s', '%s', '%s', '%s','".$_SESSION['exercicio']."')";

	$sqlUpdRpu = "UPDATE elabrev.usuarioresponsabilidade SET rpustatus = 'I' WHERE unicod = '%s' AND pflcod = ".$pflcod;
...
<?
$sql = "select distinct u.unicod as codigo, u.unicod||' - '||u.unidsc as descricao from elabrev.usuarioresponsabilidade ur inner join unidade u on ur.unicod=u.unicod where ur.rpustatus='A' and ur.usucpf = '$usucpf' and ur.pflcod=$pflcod";
$RS = @$db->carregar($sql);
if(is_array($RS)) {
	$nlinhas = count($RS)-1;
	if ($nlinhas>=0) {
