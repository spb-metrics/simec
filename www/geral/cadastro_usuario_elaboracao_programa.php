<?php
 /*
   ...
   Módulo:cadastro_usuario_elaboracao_programa.php
...
if(is_array($_POST['usuprogresp']) && @count($_POST['usuprogresp'])>0) {
	...
	$sqlSelResp = "SELECT distinct ur.rpuid, ur.usucpf, ur.rpustatus, p.prgdsc, p.prgcod, p.prgid FROM elabrev.usuarioresponsabilidade ur 
		INNER JOIN elabrev.ppaprograma_proposta p ON p.prgid = ur.prgid AND p.prgid = %s and p.prgstatus='A' 		inner join seguranca.perfil pfl on pfl.pflcod=ur.pflcod and pfl.pflsncumulativo='f' and pfl.pflcod='".$pflcod."' 			
		WHERE ur.rpustatus = 'A' AND ur.usucpf <> '" . $usucpf . "'";
	

	$sqlSelPrograma	= "SELECT p.prgid, p.prgcod FROM elabrev.ppaprograma_proposta p WHERE p.prgid = %s ";
	
	$sqlInsRpu = "INSERT INTO elabrev.usuarioresponsabilidade (prgid, usucpf, rpustatus, rpudata_inc, pflcod, prsano) VALUES ('%s', '%s', '%s', '%s', '%s','".$_SESSION['exercicio']."')";

	$sqlUpdRpu = "UPDATE elabrev.usuarioresponsabilidade SET rpustatus = 'I' WHERE prgid = '%s' AND pflcod = ".$pflcod;

	$sqlUpdRpuUsu = "UPDATE elabrev.usuarioresponsabilidade SET rpustatus = 'I' WHERE usucpf = '".$usucpf."' AND pflcod = ".$pflcod;

	//
	// verificar quais programas possuem outro com o mesmo perfil
...
<?
	  ...
	  $sql = "select prgid, prgcod, prgcod, prgdsc from elabrev.ppaprograma_proposta where prgstatus='A' and prsano='".$_SESSION['exercicio']."' and orgcod='".$_SESSION['ittorgao']."'  order by prgcod";
...
<?
$sql = "select distinct p.prgid as codigo, p.prgcod||' - '||p.prgdsc as descricao from elabrev.usuarioresponsabilidade ur inner join elabrev.ppaprograma_proposta p on ur.prgid=p.prgid where ur.rpustatus='A' and ur.usucpf = '$usucpf' and ur.pflcod=$pflcod and ur.prsano='".$_SESSION['exercicio']."' order by 2";
...
if(is_array($RS)) {
	...
	$sql = "select distinct p.prgid as codigo, p.prgdsc as descricao from elabrev.ppaprograma_proposta p inner join elabrev.progacaoproposto pp on p.prgid=pp.prgid and pp.usucpf='".$usucpf."' where pp.prgid is not null";
	$RS = @$db->carregar($sql);
	if(is_array($RS)) {
		$nlinhas = count($RS)-1;
		if ($nlinhas>=0) {
