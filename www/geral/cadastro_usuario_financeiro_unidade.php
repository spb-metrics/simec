...
if(is_array($_POST['usuunidresp']) && @count($_POST['usuunidresp'])>0) {
	...
	$sqlSelResp = "SELECT distinct ur.rpuid, ur.usucpf, ur.rpustatus, u.unidsc, u.unicod, u.unicod FROM financeiro.usuarioresponsabilidade ur 
	...
	$sqlInsRpu = "INSERT INTO financeiro.usuarioresponsabilidade (unicod, usucpf, rpustatus, rpudata_inc, pflcod,prsano) VALUES ('%s', '%s', '%s', '%s', '%s','".$_SESSION['exercicio']."')";

	$sqlUpdRpu = "UPDATE financeiro.usuarioresponsabilidade SET rpustatus = 'I' WHERE unicod = '%s' AND pflcod = ".$pflcod;

	$sqlUpdRpuUsu = "UPDATE financeiro.usuarioresponsabilidade SET rpustatus = 'I' WHERE usucpf = '".$usucpf."' AND pflcod = ".$pflcod;
...
<?
	  ...
	  $sql = "select unicod, unicod, unidsc from unidade where unistatus='A' and orgcod='".$_SESSION['ittorgao']."' order by unicod, unidsc";
...
<?
$sql = "select distinct u.unicod as codigo, u.unicod||' - '||u.unidsc as descricao from financeiro.usuarioresponsabilidade ur inner join unidade u on ur.unicod=u.unicod where ur.rpustatus='A' and ur.usucpf = '$usucpf' and ur.pflcod=$pflcod";
$RS = @$db->carregar($sql);
if(is_array($RS)) {
	$nlinhas = count($RS)-1;
	if ($nlinhas>=0) {
