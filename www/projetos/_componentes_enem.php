...
function arvore( $atividade, $profundidade = 0, $situacao = array(), $usuario = null , $diretorio = null, $numeracao_relativa = false, $perfis = array() ){
	...
	$atitipoenem  = recuperar_tipo_enem( $atividade );
	ob_start();
	?>
<!-- BIBLIOTECAS -->
<script
...
<table style="width:100%; border:0; padding:5px;">
	<tr>
		<td style="width:50%; text-align:left;" valign="top"><?php if( projeto_verificar_responsabilidade( $projeto, $usuario ) || ( $atividade && atividade_verificar_responsabilidade( $atividade, $usuario ) ) ): ?>
		
		<?php if( is_null($atitipoenem) ): ?>
		<?php // não exibe nenhuma forma de cadastro ?>
		<?php elseif( $atitipoenem == 'E' ): ?>
		<span title="nova atividade"
			onclick="cadastrar_atividade( <?= $atividade ? $atividade : $projeto ?>, 'P' );"
			style="cursor: pointer;"> <img align="absmiddle"
			src="/imagens/gif_inclui.gif" /> Cadastrar Processo </span>
			&nbsp;&nbsp;|&nbsp;&nbsp;
		<?php elseif( $atitipoenem == 'P' ): ?>
		<span title="nova atividade"
			onclick="cadastrar_atividade( <?= $atividade ? $atividade : $projeto ?>, 'S' );"
			style="cursor: pointer;"> <img align="absmiddle"
			src="/imagens/gif_inclui.gif" /> Cadastrar SubProcesso </span>
			&nbsp;&nbsp;|&nbsp;&nbsp;
		<span title="nova atividade"
			onclick="cadastrar_atividade( <?= $atividade ? $atividade : $projeto ?>, 'A' );"
			style="cursor: pointer;"> <img align="absmiddle"
			src="/imagens/gif_inclui.gif" /> Cadastrar Atividade </span>
			&nbsp;&nbsp;|&nbsp;&nbsp;
		<?php elseif( $atitipoenem == 'S' ): ?>
		<span title="nova atividade"
			onclick="cadastrar_atividade( <?= $atividade ? $atividade : $projeto ?>, 'A' );"
			style="cursor: pointer;"> <img align="absmiddle"
			src="/imagens/gif_inclui.gif" /> Cadastrar Atividade </span>
		...
		&nbsp;&nbsp;|&nbsp;&nbsp; <span
			...
			style="cursor: pointer;"> Exportar Tudo </span> &nbsp;&nbsp;|&nbsp;&nbsp; <span
			title="Abrir tudo"
			onclick="abrir_tudo();"
			style="cursor: pointer;"> <b>Abrir</b> </span> / <span
			title="Abrir tudo"
			onclick="fechar_tudo();"
			style="cursor: pointer;"> <b>Fechar</b> </span> Tudo 
		...
		<tr><td><b>Legenda :</b></td><td><td><b>E</b> - Etapa</td><td><b>P</b> - Processo</td><td><b>S</b> - Subprocesso</td><td><b>A</b> - Atividade</td></tr>
		</table>
		
		</td>
		<td style="width:25%; text-align:right;" valign="top" nowrap>
		<img
...
<script>
		...
		var URL_AJAX = 'http://<?= $_SERVER['SERVER_NAME'] ?>/<?= $diretorio ?>/ajax_enem.php'
			+ '?atividade=<?= $atividade ?>'
...
<script language="javascript" type="text/javascript">
		...
		function cadastrar_atividade( atividade, tipo ){
			if ( requisicao_acao != null ) {
				return;
			}

			if(tipo == 'P')
				var titulo = window.prompt( 'Título do Processo:', 'Novo Processo' );
			if(tipo == 'S')
				var titulo = window.prompt( 'Título do SubProcesso:', 'Novo SubProcesso' );
			if(tipo == 'A')
			...
			if ( titulo ) {
				...
				requisicao_acao.open( 'GET', URL_AJAX + '&evento=arvore_inserir&atiidpai=' + atividade + '&atidescricao=' + escape( titulo ) + '&atitipoenem=' + tipo, true );
...
<?php if( $atividade['atitipoenem'] != 'E' ): ?>

<?php if( $atividade['atitipoenem'] == 'P' ): ?>
<a href="?modulo=principal/atividade_enem/atividade&acao=P&atiid=<?= $atividade['atiid'] ?>">
<img align="absmiddle" src="../imagens/alterar.gif" style="border: 0;" title="Informações Gerais"/>
</a>
<?php endif;?>
<?php if( $atividade['atitipoenem'] == 'S' ): ?>
<a href="?modulo=principal/atividade_enem/atividade&acao=S&atiid=<?= $atividade['atiid'] ?>">
<img align="absmiddle" src="../imagens/alterar.gif" style="border: 0;" title="Informações Gerais"/>
</a>
<?php endif;?>
<?php if( $atividade['atitipoenem'] == 'A' ): ?>
<a href="?modulo=principal/atividade_enem/dados_atividade&acao=A&atiid=<?= $atividade['atiid'] ?>">
<img align="absmiddle" src="../imagens/alterar.gif" style="border: 0;" title="Informações Gerais"/>
</a>
<?php endif;?>
...
if(temPerfilAdministrador()) $permissao = true;

?>
<!-- Cadastrar Processo -->
<?php if( $permissao ): ?>
	<?php if( $atividade['atitipoenem'] == 'E' ): ?>
		<img align="absmiddle" border="0" src="../imagens/gif_inclui.gif" onclick="cadastrar_atividade( <?= $atividade['atiid'] ?>, 'P')" title="Cadastrar Processo" style="cursor:pointer;"/>
	<?php endif;?>
<?php else: ?>
	<img align="absmiddle" src="../imagens/gif_inclui_d.gif"/>
<?php endif; ?>
<!-- Cadastrar SubProcesso ou Atividade -->
<?php if( $permissao ): ?>
	<?php if( $atividade['atitipoenem'] == 'P' ): ?>
		<img align="absmiddle" border="0" src="../imagens/gif_inclui.gif" onclick="cadastrar_atividade( <?= $atividade['atiid'] ?>, 'S')" title="Cadastrar SubProcesso" style="cursor:pointer;"/>
		<img align="absmiddle" border="0" src="../imagens/gif_inclui.gif" onclick="cadastrar_atividade( <?= $atividade['atiid'] ?>, 'A')" title="Cadastrar Atividade" style="cursor:pointer;"/>
	<?php endif;?>
<?php else: ?>
	<img align="absmiddle" src="../imagens/gif_inclui_d.gif"/>
	<img align="absmiddle" src="../imagens/gif_inclui_d.gif"/>
...
$possuiItens = $db->pegaUm("SELECT iclid FROM projetos.itemchecklist WHERE atiid='".$atividade['atiid']."'");

if( $possuiItens ): 
?>
<img align="absmiddle" src="../imagens/excluir_01.gif" onclick="alert('Esta atividade possui itens cadastrados');" />
<?php elseif( $permissao && !$tem_filhos ) : ?>
<img align="absmiddle" border="0" src="../imagens/excluir.gif" onclick="excluir_atividade( <?= $atividade['atiid'] ?> )" title="Excluir Atividade" style="cursor:pointer;"/>
<?php else : ?>
<img align="absmiddle" src="../imagens/excluir_01.gif"/>
<?php endif; ?>
<a href="/<?= $diretorio ?>/gantt.php?atiid=<?= $atividade['atiid'] ?>" target="_blank"><img align="absmiddle" border="0" src="../imagens/lupa_grafico.gif" title="Exibir Gráfico" style="cursor:pointer; margin-left:5px;" /></a>
...
if( $atividade['qtdrestricoes'] > 0 ): ?> <a href="?modulo=principal/atividade_enem/controle&acao=A&atiid=<?= $atividade['atiid'] ?>"><img src="../imagens/restricao.png" border="0" align="absmiddle" title="<?= $atividade['qtdrestricoes'] ?> <?= $atividade['qtdrestricoes'] == 1 ? 'restrição' : 'restrições' ?>"/></a><?php endif; ?><?php
if( $atividade['qtdanexos'] > 0 ): ?> <a href="?modulo=principal/atividade_enem/instrumento&acao=A&atiid=<?= $atividade['atiid'] ?>"><img src="../imagens/clipe.gif" border="0" align="absmiddle" title="<?= $atividade['qtdanexos'] ?> <?= $atividade['qtdanexos'] == 1 ? 'anexo' : 'anexos' ?>"/></a><?php endif;
...
<span style="margin: 0 5px 0 0; <?= $permissao ? '' : 'color:#909090;' ?>"><?php
	...
	<?php if( $atividade['atitipoenem'] == 'E' ): ?>
	<a id="link<?= $atividade['atiid'] ?>" href="?modulo=principal/atividade_enem/subatividades&acao=E&atiid=<?= $atividade['atiid'] ?>" onmouseover="window.SuperTitleOn( this , '<b>' + this.innerHTML + '</b><?= $descricao ?>' )" onmouseout="window.SuperTitleOff( this )" style="color:#000099">(E)
	<?= $atividade['atidescricao'] ?>
	</a>
	<?php elseif( $atividade['atitipoenem'] == 'P' ): ?>
	<a id="link<?= $atividade['atiid'] ?>" href="?modulo=principal/atividade_enem/subatividades&acao=P&atiid=<?= $atividade['atiid'] ?>" onmouseover="window.SuperTitleOn( this , '<b>' + this.innerHTML + '</b><?= $descricao ?>' )" onmouseout="window.SuperTitleOff( this )" style="color:#0033FF">(P)
	<?= $atividade['atidescricao'] ?>
	</a>
	<?php elseif( $atividade['atitipoenem'] == 'S' ): ?>
	<a id="link<?= $atividade['atiid'] ?>" href="?modulo=principal/atividade_enem/subatividades&acao=S&atiid=<?= $atividade['atiid'] ?>" onmouseover="window.SuperTitleOn( this , '<b>' + this.innerHTML + '</b><?= $descricao ?>' )" onmouseout="window.SuperTitleOff( this )" style="color:#006600">(S)
	<?= $atividade['atidescricao'] ?>
	</a>
	<?php elseif( $atividade['atitipoenem'] == 'A' ): ?>
	<a id="link<?= $atividade['atiid'] ?>" href="?modulo=principal/atividade_enem/subatividades&acao=A&atiid=<?= $atividade['atiid'] ?>" onmouseover="window.SuperTitleOn( this , '<b>' + this.innerHTML + '</b><?= $descricao ?>' )" onmouseout="window.SuperTitleOff( this )" style="color:#009933">(A)
	<?= $atividade['atidescricao'] ?>
	</a>
	<?php endif; ?>
...
?>
<td <? if(!$possuiItens){ echo $onclick; } ?> id="situacao_atividade_<?= $atividade['atiid'] ?>" <?=$metaNum?> percentual="<?= $percentual?>" status="<?= (integer) $atividade['esaid'] ?>" style="text-align:center;">
<?= montar_barra_execucao( $atividade, $permissao ) ?>
</td>

<?php $onclick = $permissao ? 'onclick="montaCalendario( this );"' : ''; ?>
<td <? echo (($atividade['atitipoenem'] != 'A' && $atividade['esaid'] == STATUS_CONCLUIDO)?$onclick:''); ?> <?= $permissao ? 'title="Alterar data de início"' : '' ?> id="data_inicio_atividade_<?= $atividade['atiid'] ?>" style="text-align:center; color: <?= $permissao ? '#008000' : '#909090' ?>;">
...
?><td <? echo (($atividade['atitipoenem'] != 'A')?$onclick:''); ?> id="data_fim_atividade_<?= $atividade['atiid'] ?>" style="text-align:center; <?= $permissao ? ( strtotime( $atividade['atidatafim'] ) < time() ? 'color:#ff2020;font-weight:bold;' : 'color:#008000;' ) : 'color:#909090;' ?>;">
<?=
...
function montar_formulario_pesquisa( $foco = false ){
	...
	$formulario = sprintf(
	'<form action="/%s/%s.php?modulo=principal/atividade_enem/pesquisa&acao=A" method="post" name="pesquisa">
...
function montar_rastro_atividade( $numero, $destacar = true ){
	...
	foreach ( $lista as $indice => $item )
	{
		...
		{
			$htm = sprintf(
			'<p style="color:'.$color.';margin: 0 0 5px %dpx;font-weight:bold;font-size:120%%;">%s %s %s %s</a></p>',
			$indice * 20,
		...
		{
			$htm = sprintf(
			'<p style="margin: 0 0 5px %dpx;">%s<a style="color:'.$color.';" href="?modulo=%s&acao=%s&atiid=%d">%s %s %s</a></p>',
			$indice * 20,
			$indice != 0 ? '<img src="../imagens/seta_filho.gif" align="absmiddle" border="0">&nbsp;' : '',
			$_REQUEST['modulo'],
			$item['atitipoenem'],
...
function montar_titulo_projeto( $atividade = '' ){
	...
	global $db;
	// pega o projeto
	$sql = sprintf( "select atiid, atidescricao from projetos.atividade where atiid = %d", $_SESSION['projeto'] );
