...
class AjaxFileuploader {
	...
	public function showFileUploader($uploaderId) {
		...
		if (in_array($uploaderId, $this->uploaderIdArray)) {
			...
			return '<form id="formName'.$uploaderId.'" method="post" enctype="multipart/form-data" action="gestaopessoa.php?modulo=principal/imageupload&acao=I&dirname='.$this->uploadDirectory.'" target="iframe'.$uploaderId.'">
						...
						<iframe name="iframe'.$uploaderId.'" src="gestaopessoa.php?modulo=principal/imageupload&acao=I" width="3cm" height="4cm" style="display:none"> </iframe>
					</form>';
		}
	}
}
